...
import openpyxl
#RIGHT MODEL/
folder_path = 'D:/NUMBER_DENSITY/NO_OBS_30W_v35_40000'  # 文件夹路径
...
def extract_and_export_to_excel(folder_path, output_excel_path):
    try:
        ...
        print(f"成功找到 FLUX_OUTLET 文件: {file_1}")
        print(f"成功找到 FLUX_OUTLET_fluidSpecies 文件: {file_2}")
        print(f"成功找到 FLUX_INLET 文件: {file_3}\n")
        ...
        for index, row in df.iterrows():
            if row[0] >= 10:  #设置稳态开始时间，本次实验从10s开始，因此检索第一行时间大于10
        ...
        if not found:
            ...
            print(f"\nError:读取到的文件数未记录10s后的据，已记录的最大时间为为：{max_value:.7f}s\n")
