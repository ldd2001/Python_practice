...
# 在代码的开头设置文件夹路径，方便修改
base_dir = r"C:/Users/ldd20/Desktop/INLET_V40"  # 请修改为你的目标文件夹路径
...
def rename_folders_and_files(base_dir):
    ...
    # 第二步：遍历文件夹，重命名所有包含 "v20" 的文件夹
    for root, dirs, files in os.walk(base_dir):
        for dir_name in dirs:
            if 'v35' in dir_name:
                old_dir_path = os.path.join(root, dir_name)
                new_dir_name = dir_name.replace('v35', 'v40')
                new_dir_path = os.path.join(root, new_dir_name)
                os.rename(old_dir_path, new_dir_path)
                print(f"重命名文件夹: {old_dir_path} -> {new_dir_path}")

    # 第三步：遍历文件，重命名所有包含 "v25" 的文件
    for root, dirs, files in os.walk(base_dir):
        for file_name in files:
            if 'v35' in file_name:
                old_file_path = os.path.join(root, file_name)
                new_file_name = file_name.replace('v35', 'v40')
